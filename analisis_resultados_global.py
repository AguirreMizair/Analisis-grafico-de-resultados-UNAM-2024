# -*- coding: utf-8 -*-
"""
Created on Thu Oct 24 12:29:52 2024
...
grupo = datos.groupby('Area')['Aciertos']
...
fig.update_layout(
    title='Media de Aciertos por Area con Intervalos de Confianza del 95%',
    xaxis_title='Media de Aciertos',
    yaxis_title='Area'
)

# Guardamos el gráfico como archivo HTML interactivo
fig.write_html("Global/grafico_aciertos_area_ci.html")

#%% Grafico de puntos, por area

# Filtrar el 10% de los resultados más altos de la columna 'aciertos'
top_10_percent = datos[datos['Aciertos'] >= datos['Aciertos'].quantile(0.90)]
...
for i, area in enumerate(areas):  # Enumerar las áreas para asignar un índice numérico
    ...
    datos_area = top_10_percent[top_10_percent['Area'] == area]
    
    # Añadir jitter al eje X usando np.random.normal para crear un pequeño desplazamiento
    jitter = np.random.normal(0, 0.1, len(datos_area))  # Desplazamiento con media 0 y desviación estándar 0.1
    
    # Añadir un scatter plot para el área actual con jitter
    fig.add_trace(go.Scatter(
        x=np.repeat(i, len(datos_area)) + jitter,  # Usamos el índice numérico en lugar del nombre del área
        y=datos_area['Aciertos'],  # Valores de aciertos en el eje Y
...
fig.update_layout(
    title='Distribución de aciertos en el 10% más alto por Area',
    xaxis_title='Área',
    xaxis=dict(
...
# Guardamos el gráfico como archivo HTML interactivo
fig.write_html("Global/mejores_area_p.html")
...
for area in areas:
    # Filtrar los datos por área
    datos_area = top_10_percent[top_10_percent['Area'] == area]['Aciertos'].dropna()
    
    # Añadir el gráfico de violín para esta área
    fig.add_trace(go.Violin(
        y=datos_area,  # Datos de aciertos
        x=[area] * len(datos_area),  # Colocar el área en el eje X
...
# Actualizar el layout
fig.update_layout(
    title='Distribución de aciertos (Del 10% mas alto) por área',
    xaxis_title='Área',
    yaxis_title='Aciertos',
    showlegend=False
)
...
# Guardamos el gráfico como archivo HTML interactivo
fig.write_html("Global/mejores_area_violin.html")
...
# Actualizar el layout
fig.update_layout(
    title='Porcentaje de aspirantes (Del 10% más alto) en cada carrera',
    xaxis_title='Carrera',
...
# Guardamos el gráfico como archivo HTML interactivo
fig.write_html("Global/mejores_por_carrera.html")
...
for area in ['Area 2', 'Area 3', 'Area 1', 'Area 4']:
    # Filtrar los datos para el área actual
    datos_area = datos[datos['Area'] == area]
    
    # Añadir un histograma de la columna 'aciertos' para el área actual
    fig.add_trace(go.Histogram(x=datos_area['Aciertos'], name=f'Área {area}', opacity=0.75))
...
# Guardamos el gráfico como archivo HTML interactivo
fig.write_html("Global/histogramas.html")
